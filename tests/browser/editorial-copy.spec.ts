...
const notes = generated.filter((city) => ["new-york", "washington", "maine", "santa-cruz"].includes(city.slug))
  .map((city) => ({ ...city, note: editorial.collections[city.sourceFolderId as keyof typeof editorial.collections].note! }));
...
for (const width of [320, 390, 1440]) {
  test(`personal stories are server-rendered and fit at ${width}px`, async ({ page }, info) => {
    ...
    for (const city of notes) expect(html).toContain(city.note);
    ...
    for (const city of notes) {
      ...
      const copy = page.locator(`#city-${city.slug}`).getByText(city.note, { exact: true });
